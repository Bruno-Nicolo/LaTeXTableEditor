  ...
  var campaign = {
    ...
    table = new (function () {
      ...
      this.importData = function (content, format) {
        ...
        if (format == "auto") {
          ...
          if (/(\\begin{|\\halign|\\valign|\\ctable({|\[))/.test(content)) {
            try {
              ...
              try {
                ...
                alert(
                  "Your file was detected as LaTeX but could not be loaded."
                );
                if (window.console) {
                  console.error(e);
                }
          ...
          try {
            ...
            alert(
              "Your JSON file could not be loaded. Be sure the JSON was generated by this generator"
            );
          }
      ...
      this.selectFormat = function (format) {
        ...
        document.getElementById("latex-opt-shortcuts").style.display =
          format == "latex" ? "block" : "none";
      };
      ...
      this.separators = function (hm) {
        $id("info_diag_zero").classList.remove("active");
        $id("info_diag_one").classList.remove("active");
        $id("info_diag_two").classList.remove("active");
        if (hm == 2) {
          $id("info_diag_two").classList.add("active");
          this.twoDiagonals();

          $id("info-diagonal-block").style.display = "block";
        } else if (hm == 1) {
          $id("info_diag_one").classList.add("active");
          this.diagonal();

          $id("info-diagonal-block").style.display = "block";
        } else {
          $id("info_diag_zero").classList.add("active");
          ...
          this.forEachSelectedCell(function (cell) {
            ...
            if (
              cell.hasAttribute("data-two-diagonals") ||
              cell.hasAttribute("data-diagonal")
            ) {
      ...
      this.interpret = function (format) {
        document.getElementById("c").value =
          this.interpreters[format].call(this);
      };
      this.deleteContent = function (force) {
        force =
          force ||
          confirm(
            "Are you sure you want to delete the content of your table ?"
          );
        if (force) {
      ...
      this.importExcel = function (file) {
        ...
        if (file) {
            ...
            try {
              ...
              if (/origin.+null/.test(e.message)) {
                elem_status.innerHTML =
                  "Impossible to load worksheets locally with this browser. Use another browser or load this page in a server.";
              } else {
                console.error(e);
          ...
          reader.onload = function (e) {
            elem_status.innerHTML =
              'Converting... <a href="#" id="worksheet-cancel">Cancel</a>';
            document.getElementById("worksheet-cancel").addEventListener(
              "click",
              function (e) {
                e.preventDefault();
                worker.terminate();
      ...
      this.split = function () {
        ...
        var _this = this;
        this.Table.split(
          document.querySelectorAll("#table td[data-selected]"),
          function (cell) {
            _this.applyToCell(cell);
          }
        );
      ...
      this.textColor = function (color) {
        if (color) {
          ...
          return (
            this._id("text-color-button").getAttribute("data-color") ||
            "#000000"
          );
        }
      };
        ...
        this.forEachSelectedCell(function (cell) {
          if (!cell.hasAttribute("data-diagonal")) {
            if (cell.hasAttribute("data-two-diagonals")) {
              ...
              if (
                !(cell.querySelector(".outer > div").innerText || "").trim()
              ) {
                cell.querySelector(".outer > div").innerHTML = "[1]";
              }
              cell.querySelector(".outer").innerHTML +=
                "<div contenteditable>" +
                (cell.getAttribute("data-diagonal-data") || "[2]") +
                "</div>";
            }
        ...
        this.forEachSelectedCell(function (cell) {
          if (!cell.hasAttribute("data-two-diagonals")) {
            ...
            if (cell.hasAttribute("data-diagonal")) {
              ...
              div.innerHTML =
                "<div contenteditable>" +
                (cell.getAttribute("data-two-diagonals-data") || "[1]") +
                "</div>" +
                div.innerHTML;
            } else {
              if (
                ...
                cell.querySelector(".outer > div").innerHTML = "[2]";
              }
              div.innerHTML =
                "<div contenteditable>" +
                (cell.getAttribute("data-two-diagonals-data") || "[1]") +
                ...
                div.innerHTML +
                "<div contenteditable>" +
                (cell.getAttribute("data-diagonal-data") || "[3]") +
                "</div>";
            }
      ...
      this.merge = function (name) {
        this.statesManager.registerState();
        var _this = this;
        this.Table[name || "merge"](
          document.querySelectorAll("#table td[data-selected]"),
          function (colspan, rowspan, keep, removed) {
      ...
      this.setAlign = function (value) {
        ...
        $id("info_align_decimal").classList.remove("active");
        this.forEachSelectedCell(function (cell) {
      ...
      this.setVAlign = function (value) {
        ...
        $id("info_align_top").classList.remove("active");
        $id("info_align_middle").classList.remove("active");
        $id("info_align_bottom").classList.remove("active");
        if (value != "m") {
          this.forEachSelectedCell(function (cell) {
      ...
      this.updateLaTeXInfoCell = function (cell) {
        cell = cell || this.selectedCell;
        if (cell) {
          document.querySelector("#latex_content").value =
            this.generateForCell(cell);
        }
      };
      this.getHTML = (function () {
            ...
            if (node.nodeType == 3) {
              ...
              if (
                ...
                if (cont.tagName == "UL") {
                  ...
                  cont.lastElementChild.tagName == "UL"
                ) {
                ...
                (tagName == "FONT" && node.hasAttribute("color")) ||
                node.style.color
              ) {
                var rgba = toRGBA(node.color || node.style.color) || [
                    0, 0, 0, 1,
                  ],
                  color =
                    "#" +
                ...
                do {
                  if (
                    (trav.tagName == "FONT" && trav.hasAttribute("color")) ||
                    trav.style.color
                  ) {
                    var rgba2 = (
                      toRGBA(trav.color || trav.style.color) || [0, 0, 0, 1]
                    ).join(",");
                    ok = rgba != rgba2;
                    break;
              ...
              if (true) {
                ...
                if (
                  node.style.textDecoration &&
                  node.style.textDecoration.indexOf("line-through") > -1
                ) {
        ...
        return function getHTML(cell, n) {
          ...
          var html = cont.innerHTML
            .replace(
              /<\/(b|i)\s*>(\s*)<\s*(b|i)\s*>/gi,
              function (full, close, space, open) {
          ...
          if (/<br[^a-z>]*>/i.test(html)) {
            var opentags = [],
              html = html.replace(
                /<\s*(\/?)\s*(br|b|i|u|font\s+[^>]*|font)[^a-z>]*>/gi,
                function (full, close, tag) {
                  tag = tag.toLowerCase();
                  if (tag == "br") {
      ...
      this.selectCell = function (element, CTRL, SHIFT) {
        if (
          ...
          !this.lastSelectedCell.offsetWidth
        ) {
          ...
          for (
            var i = Math.min(posThis.y, posLast.y);
            i <= Math.max(posThis.y, posLast.y);
            i++
          ) {
            var cells = rows[i];
            for (
              var j = Math.min(posThis.x, posLast.x);
              j <= Math.max(posThis.x, posLast.x);
              j++
            ) {
              var cell = cells[j];
      ...
      this.showInfo = function (element) {
        ...
        $id("info-diagonal-color").value =
          element.getAttribute("data-diagonal-color") || "#000000";
        // Align
        this._id("info_align_left").classList.remove("active");
        this._id("info_align_center").classList.remove("active");

        this._id("info_align_decimal").classList.remove("active");
        ...
        if (align == "c") {
          ...
          this._id("info_align_decimal").classList.add("active");
        } else {
          this._id("info_align_left").classList.add("active");
        }
        // Vertical align
        this._id("info_align_top").classList.remove("active");
        this._id("info_align_middle").classList.remove("active");

        this._id("info_align_bottom").classList.remove("active");
        align = element.getAttribute("data-vertical-align");
        if (align == "t") {
          this._id("info_align_top").classList.add("active");
        } else if (align == "b") {
          this._id("info_align_bottom").classList.add("active");
        } else {
          this._id("info_align_middle").classList.add("active");
        }
        // Rotated ?
        if (element.hasAttribute("data-rotated")) {
          this._id("info-rotated").classList.add("active");
        ...
        if (
          color == "transparent" ||
          /rgba?\s*\(\s*\d+[\s,]+\d+[\s,]+\d+[\s,]+0\s*\)/.test(color)
        ) {
          color = [255, 255, 255, 0];
        } else {
          color = toRGBA(color);
      ...
      this.mode = function (n) {
        if (arguments.length == 0) {
          ...
          $id("button-mode-border").classList.remove("active");
          $id("button-mode-border").setAttribute("aria-pressed", "false");
          $id("button-mode-view").classList.remove("active");
          ...
          if (n == 1 || n == 2) {
            ...
            if (n == 1) {
              ...
              $id("button-mode-view").classList.add("active");
              $id("button-mode-view");
              ...
              $id("button-mode-border").classList.add("active");
      ...
      this.findReplace = function () {
        ...
        if (text) {
          ...
          var elements;
          if (mode == "1") {
            elements = this.element.querySelectorAll(
              "td[data-selected] div[contenteditable]"
            );
          } else if (mode == "2") {
            elements = this.element.querySelectorAll(
              "td:not([data-selected]) div[contenteditable]"
            );
          } else {
      ...
      this.message = function (text, type) {
        ...
        if (type == "warning" || type == "1") {
          this.log +=
            "<table><tr><td><span class='glyphicon glyphicon-exclamation-sign' style='padding-right:4px;'></span></td><td>" +
            text +
            "</td><tr></table><hr>";
        } else {
      ...
      this.importFromJSON = function (o) {
        ...
        document.getElementById("copt-break").checked =
          document.getElementById("opt-latex-split").checked;
        document.getElementById("copt-shrink").checked =
          document.getElementById("opt-fit-table").value.indexOf("sh") > -1;
        document.getElementById("copt-ltx-com").checked =
          document.getElementById("opt-latex-command").checked;
        if (o.caption) {
        ...
        for (var i = 0; i < o.cells.length; i++) {
          ...
          for (var j = 0; j < row.length; j++) {
            ...
            if (cellO.dataset.twoDiagonals) {
              ...
              cell.querySelector(".outer").innerHTML +=
                "<div contenteditable>" + cellO.html[1] + "</div>";
      ...
      this.exportToJSON = function (useHTML) {
        ...
        for (var i = 0, option; i < options.length; i++) {
          option = options[i];
          o.options[option.id.substring(option.id.indexOf("-") + 1)] =
            option.type == "radio" || option.type == "checkbox"
              ? option.checked
              : option.value;
        }
        o.options.latexEnv = document
          .getElementById("env-button")
          .innerText.trim();
      ...
      this.isBorderSet = function (element, where) {
        ...
        return (
          element.getAttribute("data-border-" + where.toLowerCase()) ==
            document.getElementById("border").value &&
          areSameColors(
            element.style["border" + where2 + "Color"],
            document.getElementById("border-color").getAttribute("data-value")
          )
        );
      };
      ...
      this.borderStyle = function (style, index, where) {
        style = (
          style || document.getElementById("border").value
        ).toLowerCase();
      ...
      this.backgroundPick = function () {
        var _this = this;
        ColorPicker.get(
          this._id("info-background-color").value,
          function (hex) {
            if (hex) {
      ...
      this.borderPick = function () {
        ...
        ColorPicker.get(
          borderColor.style.borderBottomColor || "#000000",
          function (color) {
      ...
      this.refreshFootnotes = function () {
        ...
        for (var i = 0, el; i < els.length; i++) {
          ...
          el = document.querySelector(
            '.tb-footnote[data-footnote-id="' + id + '"]'
          );
          if (el) {
      ...
      this.moveRow = function (beforeIndex, afterIndex) {
        ...
        for (var j = 0; j < 2; j++) {
          ...
          for (var i = 0; i < matrix[0].length; i++) {
            ...
            if (cell.cell.rowSpan > 1) {
              if (
                Math.min(beforeIndex, afterIndex) < cell.y ||
        ...
        table.Table.removeRow(
          beforeIndex > afterIndex ? beforeIndex + 1 : beforeIndex
        );
      };
        ...
        outside: for (var j = 0; j < 2; j++) {
          ...
          for (var i = 0; i < matrix.length; i++) {
            ...
            if (cell.cell.colSpan > 1) {
              if (
                Math.min(beforeIndex, afterIndex) < cell.x ||
      ...
      this.load = function (table) {
        ...
        if (window.MutationObserver) {
          var observer = new MutationObserver(
            _this.footnoteObserver.bind(_this)
          );
        ...
        table.addEventListener("mousedown", function (e) {
          if (!document.body.hasAttribute("data-border-editor")) {
            ...
            if (
              (target.tagName == "TD" && target.parentElement.rowIndex === 0) ||
              target.tagName == "TR"
            ) {
              ...
              var removeOverlay = function () {
                ...
                if (target.tagName == "TR") {
                  ...
                  if (actualTarget) {
                    ...
                    var index =
                      "cellIndex" in actualTarget
                        ? actualTarget.cellIndex + 1
                        : 0;
              ...
              var findTarget = function (e2) {
                if (target.tagName == "TR") {
                  var element = document.elementsFromPoint(
                    e.clientX,
                    e2.clientY
                  )[1];
                  var actualTarget = document.querySelector(".moveTargetY");
            ...
            if (selections.rangeCount > 1) {
              for (var i = 0; i < selections.rangeCount; i++) {
                ...
                if (ancestor.tagName == "TR") {
                  if (
                    ancestor.parentElement.id == "table" ||
              ...
              if (dataHtml) {
                ...
                if (pastedtable) {
                  ...
                  if (!newtext) {
                    ...
                    for (
                      var i = 0;
                      i < pos.x + mtable[0].length - afcol.length;
                      i++
                    ) {
                    ...
                    for (
                      var i = 0;
                      i < pos.y + mtable.length - amatrix.length;
                      i++
                    ) {
                      var alcell = alrow[0];
        ...
        table.addEventListener(
          ...
          function (e) {
            ...
            if (
              _this.selectedCell === (target.parentElement || {}).parentElement
            ) {
        ...
        this._id("worksheet-drop").addEventListener(
          "drop",
          function (e) {
            e.stopPropagation();
            e.preventDefault();
        ...
        if (document.querySelector(".toolbar-group-container")) {
          // New code
          var toolbarContainer = document.querySelector(
          ...
          toolbarContainer.addEventListener(
            ...
            function (e) {
              if (e.keyCode == 37) {
                ...
                var btns = toolbarContainer.querySelectorAll(
                  ".toolbar-group-visible button, .toolbar-group-visible .btn-caret[aria-expanded='true'] + #font-color-picker td[tabindex]"
                );
                if (!focus) {
                  btns[0].focus();
                } else {
                ...
                // Right
                var focus = toolbarContainer.querySelector(":focus"),
                  btns = toolbarContainer.querySelectorAll(
                    ".toolbar-group-visible button, .toolbar-group-visible .btn-caret[aria-expanded='true'] + #font-color-picker td[tabindex]"
                  );
                if (!focus) {
                  btns[0].focus();
                ...
                // Home
                toolbarContainer
                  .querySelector(".toolbar-group-visible button")
                  .focus();
              } else if (e.keyCode == 35) {
                // End
                var allButtons = toolbarContainer.querySelectorAll(
                  ".toolbar-group-visible button"
                );
          ...
          toolbarContainer.querySelector("table").addEventListener(
            "mousedown",
            function (e) {
              e.preventDefault();
            },
          ...
          document.getElementById("font-color-picker").addEventListener(
            "click",
            function (e) {
              if (
                (e.target || e.originalTarget || e.srcElement).tagName == "TD"
              ) {
                e.preventDefault();
                var color = (
                  e.target ||
                ...
                if (color) {
                  ...
                  var text = (e.target || e.originalTarget || e.srcElement)
                    .innerText;
                  if (text.indexOf("Auto") >= 0) {
                    that.textColor("#000000");
                    document
                      .getElementById("text-color-button")
                      .setAttribute("data-color", "none");
                    ...
                    color = color || "#000000";
                    ColorPicker.get(
                      color == "none" ? "#000000" : color,
                      that.textColor.bind(that)
                    );
                  }
                }
              }
          ...
          document.getElementById("format-drop").addEventListener(
            "click",
            function (e) {
          ...
          document.getElementById("env-button-drop").addEventListener(
            "click",
            function (e) {
              var li = e.target || e.originalTarget || e.srcElement;
              if (li.tagName == "A") {
      ...
      this.toggleMergePicker = function (el) {
        ...
        if (el.getAttribute("aria-expanded") == "false") {
          ...
          mergePicker.style.left =
            el.offsetLeft -
            document.querySelector(".toolbar-group-container").scrollLeft +
            "px";
      ...
      this.toggleFontColorPicker = function (el) {
        ...
        if (el.getAttribute("aria-expanded") == "false") {
          var colors = ColorPicker.getTableColorScheme(),
            themeTD = document
              .getElementById("font-color-theme")
              .querySelectorAll("td");
      ...
      this.refreshRotatedCellSize = function (td, stopRefresh) {
        ...
        if (td.getAttribute("data-rotated") == "45") {
          td.style.width =
            Math.abs(size.width * Math.sin((-45 * Math.PI) / 180)) +
            Math.abs(size.height * Math.cos((-45 * Math.PI) / 180)) +
            6 +
            "px";
          td.style.height =
            Math.abs(size.width * Math.cos((-45 * Math.PI) / 180)) +
            Math.abs(size.height * Math.sin((-45 * Math.PI) / 180)) +
            6 +
            "px";
        } else {
      ...
      this.generateFromHTMLPerLine = function (lines, params) {
        ...
        for (var i = 0; i < lines.length; i++) {
          ...
          if (params.numberColumn || params.number) {
            ...
            if (nb) {
              console.log(
                params.siunitx + "|" + lines.length + "|" + div.innerHTML
              );

              if (
                params.tablenum ||
                ((params.siunitx || params.siunitx === "") && lines.length > 1)
              ) {
                lineCode += "\\tablenum";
                if (params.siunitx) {
          ...
          if (!lineCode) {
            lineCode = this.generateFromHTML(
              lines[i].html,
              params.ignoreMultiline,
              params.align
            );
          }
          if (!lineCode.trim() && lines.length > 1) {
            ...
            if (lines[i].level > 0) {
              ...
              str +=
                "\\labelitem" + ["i", "ii", "iii", "iv"][lines[i].level - 1];
              if (!lines[i].item) {
                str += "}";
              }
        ...
        if (
          (params.nonNumberFormat || params.nonNumberFormat === "") &&
          (!isANumber || lines.length > 1)
        ) {
      ...
      this.generateFromHTML = function (html, ignoreMultiline, align, oldEq) {
        ...
        for (var i = 0; i < ul.length; i++) {
          ...
          if (
            !uli.parentElement ||
            uli.parentElement.tagName == "LI" ||
            uli.parentElement.tagName == "UL"
          ) {
            continue;
          }
          ...
          for (var j = 0; j < li.length; j++) {
            ...
            lis.push({
              html: this.generateFromHTML(liHTML, false, align, eq),
              level: level,
            });
          }
          var licode = "";
          if (maxLevel == 1) {
            ...
            if (align.charAt(0) == "p") {
              ULs.push(
                "\\begin{tabular}{@{}>{\\labelitemi\\hspace{\\dimexpr\\labelsep+0.5\\tabcolsep}}" +
                  align +
                  "@{}}" +
                  licode +
                  "\\end{tabular}"
            ...
            for (var j = 0, li; j < lis.length; j++) {
              ...
              licode +=
                "{\\labelitem" + ["i", "ii", "iii", "iv"][li.level - 1] + "}";
              licode +=
                "\\hspace{\\dimexpr\\labelsep+0.5\\tabcolsep}" + li.html;
            }
            ULs.push(
              "\\begin{tabular}{@{}" +
                align +
            ...
            if (useLatex) {
              ...
              if (command) {
                html =
                  ...
                  command[1] +
                  "" +
                  html.substring(i + command[2] + 2);
                i--;
                str += command[0];
                this.uniqueLog(
                  "The editor detected LaTeX commands in your text and did not escape those. You can change this behaviour in the settings.",
                  "warning"
                );
              } else {
                str += "\\textbackslash{}";
        ...
        if (hasMultiline && !ignoreMultiline) {
          str =
            "\\begin{tabular}[c]{@{}" + align + "@{}}" + str + "\\end{tabular}";
        }
        return str;
      };
      ...
      this.generateForCell = function (cell, align, shrinkRatio, o) {
        ...
        if (cell.hasAttribute("data-two-diagonals")) {
          ...
          text = "\\diagbox";
          if (
            cell.hasAttribute("data-diagonal-color") &&
            cell.getAttribute("data-diagonal-color") != "#000000"
          ) {
            text +=
              "[linecolor=" +
              this.tabuColor(cell.getAttribute("data-diagonal-color")) +
              "]";
          }
          text +=
            "{" +
            this.generateFromHTML(this.getHTML(cell, 2)) +
            "}{" +
            this.generateFromHTML(this.getHTML(cell)) +
            "}{" +
          ...
          if (cell.rowSpan > 1) {
            if (this.blacklistPackages["makecell"]) {
              var inside = this.generateFromHTML(
                this.getHTML(cell),
                true,
                align
              );
              this.message(
                'You may have to adjust the following value in one of your rotated cell : "' +
                  cell.rowSpan +
                  '\\normalbaselineskip"',
                "warning"
              );
              text =
                "\\begin{sideways}\\begin{tabular*}{" +
                cell.rowSpan +
                "\\normalbaselineskip}{" +
                align +
                "}" +
              ...
              text =
                "\\rotcell{" +
                this.generateFromHTML(this.getHTML(cell), false, align) +
                "}";
          ...
          if (text.indexOf("\\\\") > -1) {
            ...
            (!this.blacklistPackages["multirow"] && cell.rowSpan > 1) ||
            cell.colSpan > 1
          ) {
            var text2 = "\\tablenum";
            var sioptions = [];
            if (o.decimalChars) {
              ...
              sioptions.push(
                "table-format=" + this.formatDecimalCharacters(o.globalDecimals)
              );
            }
          ...
          cell.rowSpan > 1 &&
          (!this.blacklistPackages["makecell"] || this.shrink) &&
          !this.blacklistPackages["multirow"]
        ) {
          ...
          var tb = 0;
          text = this.generateFromHTML(this.getHTML(cell), true, align).replace(
            /(?:\\{2,}|\\(?:begin|end)\{tabular\})/g,
            function (full) {
        ...
        if (background) {
          ...
          text = text.replace(
            /\\begin\{tabular\}[^@\{]*\{@\{(?:\\labelitemi\\hspace\{\\dimexpr\\labelsep\+0\.5\\tabcolsep\}|)\}/g,
            function (full) {
      ...
      this.calculateDecimalCharacters = function (
        cell,
        separator,
        falseIfError
      ) {
        ...
        if (dm.split(/\./g).length > 2) {
          separator = ",";
        } else if (dm.indexOf(",") > dm.indexOf(".")) {
          separator = ",";
        } else {
          separator = ".";
        }
      ...
      this.createCellO = function (o, row) {
        ...
        if (!this.blacklistPackages["vcell"]) {
          if ((before && before.vcell) || o.valign != "m") {
            ...
            for (var i = 0; i < row.length; i++) {
              if (
                row[i].cell &&
                (row[i].cell.getAttribute("data-vertical-align") || "m") != "m"
              ) {
                o.vcell = true;
                break;
              }
        ...
        o.getVCellContent = function (actualColor, forceMulti) {
          if (o.rowSpan != 1) {
            ...
            if (o.valign != "m") {
              _this.uniqueLog(
                "Vertical alignment of decimal-aligned cells is not supported yet.",
                "warning"
              );
            }
          } else if (o.valign == "t") {
        ...
        o.getFullContent = function (actualColor, forceMulti) {
          ...
          if (cell.rowSpan != 1 && !blockMultirow) {
            ...
            if (_this.shrink) {
              content = _this.multirow(
                ratio * cell.rowSpan,
                "\\hspace{0pt}" + content,
                o.shrinkRatio + "\\linewidth"
              );
            } else {
            ...
            if (
              o.vcell &&
              o.align != "d" &&
              !_this.blacklistPackages["vcell"]
            ) {
              content =
                "\\vcell{" +
                content.replace(/\{tabular\}\[c\]/g, "{tabular}[b]") +
                "}";
            }
            if (o.background) {
              _this.packages["colortbl"] = true;
      ...
      this.applyOddEvenColors = function () {
        this.statesManager.registerState();
        this.oddEvenColors(
          this._id("oddevencolors-number").value,
      ...
      this.showOddEvenColorsModal = function () {
        ...
        this._id("oddevencolors-even").value =
          toHex(toRGBA(actual[1])) || "#FFFFFF";
        this._id("oddevencolors-odd").value =
          toHex(toRGBA(actual[2])) || "#FFFFFF";
              ...
              cell.getFullContent = (function (cell, refCell) {
                return function (actualColor, forceMulti) {
                  ...
                  if (
                    refCell.colSpan != 1 ||
                    forceMulti ||
                    !refCell.isInPreambule
                  ) {
                    return _this.multicolumn(
                      refCell.colSpan,
                      cell.getHeader(actualColor),
                      content
                    );
                  }
                  return content;
      ...
      this.caption = function () {
        return {
          caption: $id("caption").value,
          numbered: $id("caption-nb").value == "*",
          label: $id("label").value,
        };
      };
      this.headerBorder = function (
        type,
        color,
        mainColor,
        preambule,
      ...
      this.toMWE = function () {
        ...
        if (format == "latex" && src.indexOf("\\documentclass") < 0) {
          ...
          src = src.replace(/^%\s*\\usepack/gm, "\\usepack");
          src = src.replace(/^%\s*\\UseTblrLibrary/gm, "\\UseTblrLibrary");
          src =
            "\\documentclass{article}\n" +
            (utf8 ? "\\usepackage[utf8]{inputenc}\n" : "") +
            src;
          var lastpackage = Math.max(
              src.lastIndexOf("\\usepackage"),
              src.lastIndexOf("\\UseTblrLibrary")
            ),
            endofheader = src.indexOf("}", lastpackage);
          src =
            src.substring(0, endofheader + 1) +
            "\n\n\\begin{document}\n" +
            src.substring(endofheader + 1) +
            "\n\\end{document}";
          src = src.replace(/\n{3,}/g, "\n\n");
        } else if (
          (format == "plain" || format == "eplain") &&
          !/\\bye$/.test(src)
        ) {
          src += "\n\\bye";
      ...
      this.globalDecimals = function (o, matrix) {
        ...
        for (var i = 0; i < matrix.length; i++) {
          ...
          if (
            traveledCell.x == (o.refCell || o).x &&
            traveledCell.cell.colSpan == (o.refCell || o).cell.colSpan
          ) {
      ...
      this.comparableHeader = function (before, middle, after) {
        ...
        return function (color, isPreambule) {
          var preambule = "",
            align2 = align,
            shrinkRatio = o.shrinkRatio,
            microfixBorderActivate = _this._id(
              "opt-latex-microfix-alignment"
            ).checked;
          ...
          if (microfixBorderActivate) {
            ...
            for (var i = 0; i < matrix.length; i++) {
              if ((o.refCell || o).x == 0) {
                ...
                if (microfixLeftCell) {
                  var microfixLeftBorder = (
                    microfixLeftCell.refCell || microfixLeftCell
                  ).leftBorder;
                  biggestLeftBorder = Math.max(
                    biggestLeftBorder,
              ...
              if (microfixRightCell) {
                microfixRightCell =
                  microfixRightCell.refCell || microfixRightCell;
                if (
                  microfixRightCell.x + microfixRightCell.colSpan - 1 ==
                  (o.refCell || o).x + middle.colSpan - 1
                ) {
                  var microfixRightBorder = microfixRightCell.rightBorder;
                  biggestRightBorder = Math.max(
                    biggestRightBorder,
            ...
            if (o.x == 0) {
              ...
              if (leftBorderSize < biggestLeftBorder) {
                o.leftCorrection = _this.microfixBorderCorrection(
                  leftBorderSize,
                  biggestLeftBorder,
                  o.cellBackground
                );
            ...
            if (rightBorderSize < biggestRightBorder) {
              o.rightCorrection = _this.microfixBorderCorrection(
                rightBorderSize,
                biggestRightBorder,
                o.cellBackground
              );
          ...
          if (align2 == "d") {
            ...
            if (o.span) {
              ...
              if (o.globalDecimals) {
                align2 +=
                  "[table-format=" +
                  _this.formatDecimalCharacters(o.globalDecimals);
                if (shrinkRatio) {
                  align2 +=
                    ",table-column-width=" + shrinkRatio + "\\linewidth";
                }
                align2 += "]";
              } else if (shrinkRatio) {
          ...
          if (rightBorder) {
            ...
            if (
              isPreambule &&
              _this.packages["arydshln"] &&
              !areSameColors(color, _this.actualMainColor) &&
              areSameColors(_this.actualMainColor, rightColor)
            ) {
              _this.actualColor = color = rightColor;
              before += "\\arrayrulecolor" + getColor(rightColor);
            } else if (
              rightBorder == "dottedline" ||
              rightBorder == "hdashline"
            ) {
              if (!areSameColors(rightColor, color)) {
                if (_this.useTabu) {
                  _this.uniqueLog(
                    "Some vertical lines could not be colored correctly with 'tabu' package.",
                    "warning"
                  );
                } else {
      ...
      this.autoLaTeXFormat = function () {
        ...
        if (borders.all.length > 1) {
          ...
          if (
            borders.none &&
            table.element.querySelector("td[style*='background-color']")
          ) {
            return "tabular";
          }
        }
        if (
          borders.double &&
          ($id("opt-latex-hhline").checked ||
            $id("opt-latex-hhline-hash").checked)
        ) {
          return "tabular";
        }
        if (format == "tabular") {
          var nbVertical = 0;
          nbVertical += Number(
            !!table.element.querySelector("td[data-vertical-align='t']")
          );
          nbVertical += Number(
            !!table.element.querySelector("td[data-vertical-align='b']")
          );
          if (nbVertical > 0) {
            format = "tabularray";
          }
      ...
      this.shrinkRatios = function (matrix) {
        ...
        if (this.shrink) {
          ...
          for (var i = 0; i < fakeRow.childNodes.length; i++) {
            widthArray.push(
              Math.round(
                (fakeRow.childNodes[i].scrollWidth / table.scrollWidth) * 1000
              ) / 1000
            );
          }
          for (var i = 0; i < table.rows.length; i++) {
      ...
      this.headers = function (matrix) {
        ...
        var headers = [],
          colHeaders = [],
          widthArray = this.shrinkRatios(matrix),
          _this = this,
          table = this.element;
        ...
        for (var j = 0; j < colLength; j++) {
          ...
          for (var i = 0; i < matrix.length; i++) {
            ...
            if (cell && !cell.ignore) {
              ...
              headernow[align]++;
              if (cell.vcell) {
                // If we use `vcell`, the header will be counted twice because
                // a fake row is used
                headernow[align]++;
              }
      ...
      this.shouldUseTabu = function () {
        if (this.blacklistPackages["tabu"]) {
          ...
          return (
            this.hasVBorderType("double") &&
            this.hasHBorderType("double") &&
            (this.hasBorderType("hdashline") ||
              this.hasBorderType("dottedline"))
          );
        } else {
          return false;
        }
      ...
      this.listBorderTypes = function () {
        ...
        var arr = [];
        for (
          var i = 1, rows = table.element.rows, l = rows.length;
          i < l;
          i++
        ) {
          var row = rows[i];
          for (var j = 0, cells = row.cells, ll = cells.length; j < ll; j++) {
            var cell = cells[j];
            if (
              !cell.hasAttribute("data-border-left") &&
              ...
              for (var k = 0; k < 4; k++) {
                var border = cell.getAttribute(
                  "data-border-" + ["left", "bottom", "top", "right"][k]
                );
                if (border && !o[border]) {
                  o[border] = true;
                  arr.push(border);
        ...
        for (var i = 1, rows = el.rows, l = rows.length; i < rows; i++) {
          ...
          for (var j = 0, cells = row.cells, ll = cells.length; j < l; j++) {
            ...
            if (cell.dataset) {
              ...
              if (
                data.borderLeft == type ||
                data.borderBottom == type ||
                data.borderTop == type ||
                data.borderRight == type
              ) {
                return true;
              }
            } else if (
              cell.getAttribute("data-border-left") == type ||
      ...
      this.useBooktab = function () {
        return (
          this.element.hasAttribute("data-booktabs") ||
          this.hasHBorderType("toprule") ||
          this.hasHBorderType("midrule") ||
      ...
      this.generateLaTeX = function (opt) {
        ...
        if (this.useTabu) {
          ...
          this.message(
            'Usage of <tt>tabu</tt> is <a href="https://github.com/tabu-issues-for-future-maintainer/tabu">not recommanded</a> by the LaTeX3 Project Team.',
            "warning"
          );
        }
        if (
        ...
        if (useLongtable) {
          ...
          if (scale) {
            this.message(
              "'scale' option can't be used with longtable or longtabu."
            );
          }
        } else {
          if (!float && rotateTable) {
            ...
            if (float) {
              firstPart =
                "\\begin{" + (rotateTable ? "sidewaystable" : "table") + "}\n";
            } else {
        ...
        if (!useLongtable) {
          str +=
            "\\begin{" + (useTabu ? "tabu" : "tabular") + "}{" + header + "}";
        }
        var rg2 = [],
          rowIndex = [],
          isVCell = [],
          ...
          for (var j = 0; j < cells.length; j++) {
            ...
            if (!cell || cell.ignore) {
              ...
              row.push({
                ...
                colSpan:
                  cell.colSpan ||
                  (cell.refCell ? cell.refCell.colSpan : 1) ||
                  1,
              });
            }
          }
            ...
            for (var j = 0; j < cells.length; j++) {
              ...
              if (!cell || cell.ignore) {
                ...
                if ((cell.refCell || cell).rowSpan == 1) {
                  ...
                  var refCell =
                    rg[(cell.refCell || cell).y + 1][(cell.refCell || cell).x];
                  text += refCell.getVCellContent(this.actualColor);
                }
                row.push({
                  text: text,
                  colSpan:
        ...
        // Booktabs
        if (
          /\\(bottomrule)|(toprule)|(midrule)|(cmidrule)|(heavyrulewidth)|(lightrulewidth)/.test(
            str
          )
        ) {
        ...
        if (str.indexOf("\\vcell") > -1) {
          ...
          this.uniqueLog(
            "If you get an <kbd>File 'vcell.sty' not found</kbd> error, download the file <a href='https://ctan.org/pkg/vcell' target='_blank'>here</a> and install it in the same repertory as your table.",
            "warning"
          );
        }
        if (
          str.indexOf("\\arrayrulecolor") > -1 ||
          firstPart.indexOf("\\arrayrulecolor") > -1 ||
          str.indexOf("\\doublerulesepcolor") > -1 ||
          firstPart.indexOf("\\doublerulesepcolor") > -1
        ) {
          ...
          str.indexOf("\\cellcolor") > -1 ||
          str.indexOf("\\rowcolor") > -1 ||
          str.indexOf("\\columncolor") > -1
        ) {
          this.packages["colortbl"] = true;
        }
        ...
        for (var i in this.packages) {
          if (this.packages.hasOwnProperty(i)) {
            if (i == "ulem") {
              ...
              i != "arydshln" &&
              !(i == "color" && this.packages["colortbl"])
            ) {
        ...
        if (this.shrink && this.packages["multirow"]) {
          this.message(
            "The shrink algorithmn might not work with cells spanning multiple rows.",
            "warning"
          );
        }
      ...
      this.showPackagesInformation = function (packages) {
        ...
        if (packs.length == 1) {
          this.message(
            "This table will import one package : " + packs[0] + ".",
            "info"
          );
          ...
          this.message(
            "This table will import " +
              ...
              "." +
              (hasSub
                ? "<br><small>* This package is imported by another package.</small>"
                : ""),
            "info"
          );
        }
              ...
              if (!intersectionPoints[j]) {
                if (j - colspan + 1 <= 0) {
                  ...
                  intersectionPoints[j] =
                    (intersectionPoints[j - colspan] || 0) +
                    2 +
                    cell.text.length +
                    1;
                }
              } else {
                intersectionPoints[j] = Math.max(
                  intersectionPoints[j],
                  (j === 0 ? 0 : (intersectionPoints[j - colspan] || 0) + 2) +
                    cell.text.length +
                    1
                );
              }
      ...
      this.extract = (function () {
        ...
        function backgroundInfo(cell) {
          var color =
            window
              ...
              .getPropertyValue("background-color") ||
            cell.style.backgroundColor;
          if (color == "transparent" || color == "white" || !color) {
            return "";
          }
        ...
        return function (div) {
          ...
          o.caption = {
            caption: wordCaption
              ? wordCaption.innerText || wordCaption.textContent
              : table.title || "",
            numbered: false,
            label: table.id || "",
          };
          for (var i = 0; i < table.rows.length; i++) {
            ...
            for (var j = 0; j < cells.length; j++) {
              ...
              if (
                (cell.getAttribute("style") || cell.style.cssText).indexOf(
                  "mso-diagonal"
                ) > -1
              ) {
        ...
        if (n == 0) {
              ...
              if (border[i - 1]) {
                ...
                if (areSameColors(beforeBorder.color, o.color)) {
                  if (o.type == beforeBorder.type) {
                    ...
                    (o.type == "trimright" || o.type == "trimfull") &&
                    (beforeBorder.type == "trimfull" ||
                      beforeBorder.type == "trimleft")
                  ) {
                    o.sameAsBefore = true;
                  }
                }
          ...
          for (var i = 0; i < row.length; i++) {
            ...
            if (!bd && row2[i]) {
              ...
              if (
                cell.cell ||
                cell.refCell.cell.parentElement.rowIndex - 1 == n + 1
              ) {
                (bd = (cell.cell || cell.refCell.cell).getAttribute(
                  "data-border-top"
                )),
                  (color = (cell.cell || cell.refCell.cell).style
                    .borderTopColor);
              }
            }
            if (bd) {
      ...
      this.prepareDownload = function () {
        ...
        console.log(latex);
        latex = latex.replace(/^%\s*\\usepack/gm, "\\usepack");
        latex = "\\documentclass{article}\n" + latex;
        latex =
          latex.replace(/\\begin{tabl/, "\\begin{document}\n\\begin{tabl") +
          "\n\\end{document}";
      ...
      this.prepareEmail = function () {
        ...
        this._id("link-email").href =
          "mailto:?subject=Table&body=" + encodeURIComponent(result);
        $("#email").modal("show");
      };
      this.vcell = function (n, matrix) {
        ...
        for (var i = 0; i < row.length; i++) {
          ...
          var cellalign =
            (cell.refCell || cell).cell.getAttribute("data-vertical-align") ||
            "m";
        ...
        if (arguments.length < 2) {
          if (
            document
              ...
              .classList.contains("prevent-small")
          ) {
            return 0;
        ...
        return this.HBorder(
          ...
          function (o) {
            ...
            var complete = o.complete,
              ...
              border = "",
              enhanceHhline =
                this._id("opt-latex-hhline").checked &&
                (!this.packages["arydshln"] || this.useTabu),
              hashHhline =
                this._id("opt-latex-hhline-hash").checked &&
                (!this.packages["arydshln"] || this.useTabu),
            ...
            if (complete) {
              ...
              if (
                ...
                !(
                  this.useTabu &&
                  (firstBorder.type == "hdashline" ||
                    firstBorder.type == "dottedline")
                )
              ) {
              ...
              if (
                ...
                border += "\\cmidrule(";
                if (
                  firstBorder.type == "trimleft" ||
                  firstBorder.type == "trimboth"
                ) {
                  border += "l";
                }
                if (
                  firstBorder.type == "trimright" ||
                  lastBorder.type == "trimright" ||
                  firstBorder.type == "trimboth"
                ) {
                  border += "r";
                }
                ...
                border += {
                  ...
                  hdashline: this.useTabu
                    ? "\\tabucline[on 4pt off 4pt]{-}"
                    : "\\hdashline",
                  dottedline: this.useTabu
                    ? "\\tabucline[on 1pt off 1pt]{-}"
                    : "\\hdashline[1pt/1pt]",
                }[firstBorder.type];
              }
              return border;
            ...
            if (specificTrim) {
              ...
              !this.blacklistPackages["hhline"] &&
              (useHHLine || // If there's a cell with background color
                (o.types.double && // or a double border but
                  !(
                    o.types.toprule ||
                    o.types.midrule || // without booktab borders
                    o.types.bottomrule ||
                    o.types.trim
                  ))) && // ...
              ...
              for (var i = 0; i < borders.length; i++) {
                if (i === 0) {
                  ...
                  var borderLeft = (row[i].refCell || row[i]).leftBorder,
                    borderLeftColor =
                      (row[i].refCell || row[i]).leftBorderColor || "#000",
                    borderLeftTop = (
                      (toprow[i] || {}).refCell ||
                      toprow[i] ||
                      {}
                    ...
                    borderLeftTopColor =
                      ((toprow[i] || {}).refCell || toprow[i] || {})
                        .leftBorderColor || "#000";
                  if (
                    hashHhline &&
                    (borderLeft == "double" || borderLeftTop == "double")
                  ) {
                    var borderLeftFColor =
                      borderLeftColor || borderLeftTopColor;
                    if (
                      insertInHhline &&
                      !areSameColors(insideColor, borderLeftFColor)
                    ) {
                      insideColor = borderLeftFColor;
                      border +=
                        ">{\\arrayrulecolor" + getColor(borderLeftFColor) + "}";
                    }
                    ...
                    if (
                      insertInHhline &&
                      !areSameColors(insideColor, borderLeftColor)
                    ) {
                      insideColor = borderLeftColor;
                      border +=
                        ">{\\arrayrulecolor" + getColor(borderLeftColor) + "}";
                    }
                ...
                if (
                  ...
                  if (insertInHhline) {
                    ...
                    if (
                      !areSameColors(doublerulesepcolor, [255, 255, 255, 1])
                    ) {
                      doublerulesepcolor = "FFFFFF";
                      toadd +=
                        "\\doublerulesepcolor" + getColor([255, 255, 255, 1]);
                    }
                    if (toadd) {
                      border += ">{" + toadd + "}";
                    }
                ...
                if (
                  row[i].x ==
                  (row[i].refCell || row[i]).x +
                    (row[i].refCell || row[i]).cell.colSpan -
                    1
                ) {
                  borderRight = (row[i].refCell || row[i]).rightBorder;
                  borderRightColor = (row[i].refCell || row[i])
                    .rightBorderColor;
                }
                if (
                  toprow[i] &&
                  toprow[i].x ==
                    (toprow[i].refCell || toprow[i]).x +
                      (toprow[i].refCell || toprow[i]).cell.colSpan -
                      1
                ) {
                  borderRightTop = (
                    (toprow[i] || {}).refCell ||
                ...
                if (
                  hashHhline &&
                  (borderRight == "double" || borderRightTop == "double")
                ) {
                  var borderRightFColor =
                    borderRightColor || borderRightTopColor;
                  if (insertInHhline) {
                    var toadd = "";
                  ...
                  if (
                    type == "double" ||
                    (borders[i + 1] && borders[i + 1].type == "double")
                  ) {
                    border += "#";
                  } else if (
                    !(borderRight == "double" && borderRightTop == "double")
                  ...
                  if (!row[i + 1]) {
                    if (
                      insertInHhline &&
                      !areSameColors(insideColor, borderRightColor)
                    ) {
                      insideColor = borderRightColor;
                      border +=
                        ">{\\arrayrulecolor" + getColor(borderRightColor) + "}";
                    }
              ...
              if (
                o.complete &&
                !/[:\|#]/g.test(border) &&
                borders[0].type == "double"
              ) {
                border = "\\hline\\hline";
                if (hasColor) {
                  border =
                    "\\arrayrulecolor" + getColor(firstBorder.color) + border;
              ...
              if (metAry || metTrim) {
                ...
                for (var i = 0; i < borders.length; i++) {
                  if (borders[i]) {
                    ...
                    if (
                      metAry &&
                      (type == "hdashline" || type == "dottedline")
                    ) {
                      if (firstAry) {
                        firstAry = false;
                      } else {
              ...
              for (var i = 0; i < borders.length; i++) {
                ...
                if (borderO.type == "trimboth") {
                  ...
                  borderO.type == "trimleft" ||
                  borderO.type == "trimfull"
                ) {
                  ...
                  border +=
                    {
                      ...
                      hdashline: this.useTabu
                        ? "\\tabucline[" + insideTabu + "on 4pt off 4pt]"
                        : "\\cdashline",
                      dottedline: this.useTabu
                        ? "\\tabucline[" + insideTabu + "on 1pt off 1pt]"
                        : "\\cdashline",
                      double: "\\cmidrule",
                    }[borderO.type] +
                    "{" +
            ...
            for (var j = 0; j < row.length; j++) {
              ...
              if (cell.cell) {
                ...
                if (intersectRect(posCell, rectangle)) {
                  ...
                  var objcells = { up: [], down: [], left: [], right: [] },
                    othercells = [];
                  //up
                  var subrow = matrix[i - 1],
                    subcell;
                  if (subrow) {
                    for (var h = 0; h < cell.colSpan; h++) {
                      ...
                      if (
                        subcell &&
                        (subcell.cell || subcell.x == subcell.refCell.x)
                      ) {
                  ...
                  for (var h = 0; h < cell.rowSpan; h++) {
                    ...
                    if (subrow) {
                      ...
                      if (
                        subcell &&
                        (subcell.cell || subcell.y == subcell.refCell.y)
                      ) {
                        othercells.push((subcell.refCell || subcell).cell);
                      }
                    }
            ...
            cellLoop: for (var j = 0, cell, posCell; j < row.length; j++) {
              ...
              if (intersectRect(posCell, rectangle)) {
                ...
                var othercells = [],
                  ...
                  subrow =
                    matrix[
                      where == "top" ? actualY - 1 : actualY + cell.rowSpan
                    ];
                if (subrow) {
                  for (var h = 0; h < cell.colSpan; h++) {
                    ...
                    if (
                      othercell &&
                      (othercell.cell || othercell.x == othercell.refCell.x)
                    ) {
      ...
      this.setTouchEvents = function (table_element) {
            ...
            element.style.width =
              Math.sqrt(
                (initialY - y) * (initialY - y) +
                  (initialX - x) * (initialX - x)
              ) + "px";
            var angle =
              Math.atan2(x - initialX, -(y - initialY)) * (180 / Math.PI) - 90;
        ...
        function mousedown(x, y, target) {
          ...
          if (document.body.hasAttribute("data-border-editor") && element) {
            ...
            if (
              !overElement ||
              (overElement.tagName != "TEXTAREA" &&
                overElement.tagName != "INPUT" &&
                overElement.tagName != "BUTTON" &&
                overElement.tagName != "SELECT")
            ) {
        ...
        function mousemove(x, y) {
          if (start && allowed) {
            ...
            var elementsToRemove = table.element.querySelectorAll(
              "td[data-border-preview]"
            );
            ...
            if (mode == 1) {
              ...
              for (var i = 0; i < draw.arg.length; i++) {
                ...
                arg[0].setAttribute(
                  "data-border-preview",
                  (arg[0].getAttribute("data-border-preview") || "") +
                    " " +
                    arg[1]
                );
                ...
                for (var j = 0; j < othercells.length; j++) {
                  ...
                  othercell.setAttribute(
                    "data-border-preview",
                    (othercell.getAttribute("data-border-preview") || "") +
                      " " +
                      {
                        top: "bottom",
                        bottom: "top",
        ...
        document
          .getElementById("opt-fit-table")
          .addEventListener("input", function (e) {
            document.getElementById("copt-shrink").checked =
              this.value.indexOf("sh") > -1;
          });
        document
          .getElementById("copt-ltx-com")
          .addEventListener("input", function (e) {
            document.getElementById("opt-latex-command").checked = this.checked;
          });
        document
          .getElementById("opt-latex-command")
          .addEventListener("input", function (e) {
...
window.addEventListener(
  ...
  function () {
    if (window.table) {
      ...
      if ($id("format-in")) {
        localStorage.setItem("table_format", $id("format-in").value);
      }
    }
  },
  false
