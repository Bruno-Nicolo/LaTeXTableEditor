  ...
  var generateCode = function (cell, obj, isS, width) {
    ...
    if (
      ...
      if (
        ...
        optarg +=
          "linecolor=" +
          table.tabuColor(cell.getAttribute("data-diagonal-color"));
      }
      if (width) {
        if (optarg) {
          optarg += ",";
      ...
      if (frag.childNodes[2]) {
        code +=
          "{" +
          table.generateFromHTML(frag.childNodes[2].innerHTML, true, "l") +
          "}{" +
      ...
      if (obj.align == "d") {
        options.siunitx =
          "table-format=" +
          table.formatDecimalCharacters(
            obj.globalDecimals || obj.columnDecimals
          );
      }
    ...
    this.forCommonProperties = function (keyCell, isAvailableFn, callback) {
      ...
      outside: for (var i in cell.right) {
        ...
        for (var j in values) {
          if (
            values[j] > max ||
            (values[j] == max && j.length > topvalue.length)
          ) {
            topvalue = j;
            max = values[j];
          }
    ...
    this.getBorders = function (
      ...
      if (hasAllVBorders) {
        ...
        this.forCommonProperties(
          ...
          function (border, y, x) {
            return (
              border.type == "vline" &&
              (allDoubles || border.right.type != "double")
            );
          },
          function (prop, value) {
            vlines[prop] = value;
      ...
      outside: for (var i = 0; i < borders.length; i++) {
        ...
        double: for (var k = 1; k <= 2; k++) {
          ...
          var isThere = false;
          var hasAll =
            (hasAllHBorders && border.type == "hline") ||
            (hasAllVBorders && border.type == "vline");
          if (hasAll) {
      ...
      for (var i = 0; i < allBorders.length; i++) {
        ...
        for (var j = 0; j < allBorders.length; j++) {
          ...
          if (
            ...
            if (
              comp[0] == "hline" &&
              comp[1].toString() == comp2[1].toString()
            ) {
              comp[3].push(+comp2[3].toString());
              comp[3].sort(function (a, b) {
                return a - b;
              });
              allBorders[j] = false;
            } else if (
              comp[0] == "vline" &&
              comp[3].toString() == comp2[3].toString()
            ) {
              comp[1].push(+comp2[1].toString());
              comp[1].sort(function (a, b) {
                return a - b;
              });
              allBorders[j] = false;
            }
      ...
      for (var i = 0; i < allBorders.length; i++) {
        ...
        str +=
          ...
          border[0] +
          "{" +
          arrayToRange(border[1], border[0] == "hline" ? maxY + 1 : maxX + 1) +
          "} = ";
        if (border[2] > 1) {
          str += "{" + border[2] + "}";
        }
        str +=
          "{" +
          arrayToRange(border[3], border[0] == "hline" ? maxX : maxY) +
          "}{" +
          border[4] +
          "},";
      }
    ...
    for (var i = 0; i < rg.length; i++) {
      ...
      for (var j = 0; j < row.length; j++) {
        ...
        if (cell.decimalChars && cell.cell && cell.cell.colSpan == 1) {
          hasSiUnitX = true;
          decCols[cell.x + 1] = table.formatDecimalCharacters(
            cell.globalDecimals
          );
        }
      }
      ...
      for (var j = 0; j < row.length; j++) {
        ...
        if (cell.cell) {
          ...
          if (cell.leftBorder) {
            ...
            if (
              cell.leftBorder == "toprule" ||
              cell.leftBorder == "bottomrule"
            ) {
              borderO.wd = "0.08em";
          ...
          if (cell.rightBorder) {
            ...
            if (
              cell.rightBorder == "toprule" ||
              cell.rightBorder == "bottomrule"
            ) {
              borderO.wd = "0.08em";
          ...
          var props = generateCode(
            cell.cell,
            cell,
            decCols[cell.x + 1],
            widthArray ? widthArray[cell.x + 1] : null
          );
          if (props.color) {
